...
{
    glm::mat4 m(0.0f);
    m[0][0] = 1.0f / (aspect * tan(fov / 2.0f));
...
{
    float normalizeLength = Maths::length(v);
    if (normalizeLength == 0.0f)
    {
        return glm::vec3(0.0f, 0.0f, 0.0f);
    }
    return glm::vec3((v.x / normalizeLength), (v.y / normalizeLength), (v.z / normalizeLength));
}
